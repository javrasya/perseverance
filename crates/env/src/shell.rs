...
pub enum Shell {
    ...
    /// run and the harvest degrades silently — recorded as an honest limit
...
/// The shipped command for a shell and a nonce. The payload is generated here
/// rather than passed in, so no caller — and no test — can frame a harvest the
/// shipped code would not have framed.
