...
Next, Frontier, Blocked, Resolved, Out of scope, Unclassified — and **position
in the column**. Everything a row adds beyond that, it adds in words.

*Out of scope* is the first of those that is not a state. It heads rows that are
all resolved, and its count is deliberately **not progress**: it holds the work
the map document itself cut, apart from the rest so that *Resolved* goes on
...
Two of the pane's groups now hold rows that are not work at all. *Unclassified*
is a counted section, last of the list, holding the children nobody classified —
a number there is a true claim about a real fault. The **Destination** is not a
section: it holds the spec children, it is drawn between the last section and
the fog, and it heads no count, because a count over the destination would be
counting the thing all the counting is for. Both are the same rule — kind
decides the group before state does, so a child that is not a ticket is never
grouped as available work — and it is recorded in
[ADR 0018](0018-a-child-that-is-not-a-ticket-is-not-on-the-work-scale.md).
